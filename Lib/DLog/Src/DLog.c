...
UART_HandleTypeDef *DLogUart = &huart8;
...
#endif              /* __GNUC__ */
PUTCHAR_PROTOTYPE { /* поведение функции не отслеживается в дальнейшем */
    ...
    if (*format == '%') {
      ...
      case 'd': {
        ...
        if (len < (int)size) {
          ...
          for (int i = len - 1; i >= len - (int)size + 1; i--) {
            *p++ = buf[i];
          }
          size = 0;
        }
      ...
      case 'h': {
        ...
        if (*format == 'h') {
          format++;
          unsigned char hx = (unsigned char)va_arg(ap, int);
          char buf[8];
          int len = 0;
          do {
            int digit = hx & 0xF;
